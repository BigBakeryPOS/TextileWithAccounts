...
{
    public partial class Cuttingmaster : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (name == "Add New")
            {
                add.Visible = true;
                div3.Visible = false;
                head.InnerText = "Cutting Master";
            }
            else if (name == "Bulk Addition")
            {
                add.Visible = false;
                //  div1.Visible = true;
                head.InnerText = "Bulk Customer/Cutting Master Addition";
            }
            ...
            if (!IsPostBack)
            {
               ...
               // DataSet getitemdistinct = objBs.editcuttingmasterrate(iCusID1);
                ...
                for (int ii = 0; ii < getitemdistinct.Tables[0].Rows.Count; ii++)
                {
                    ...
                    DataSet getstcrate = objBs.editcutmaster(iCusID1, itemid);
                    if (getstcrate.Tables[0].Rows.Count > 0)
                    {
                        string CutRate = getstcrate.Tables[0].Rows[0]["CutRate"].ToString();
                        dr["cutrate"] = Convert.ToDouble(CutRate).ToString("0.00");
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (iCusID != "" || iCusID != null)
                    {
                        ...
                        DataSet ds2 = objBs.editCustomerforbillseries(iCusID);
                        if (ds1.Tables[0].Rows.Count > 0)
                        {
                            ...
                            {
                                txtLotNo.Text = ds2.Tables[0].Rows[0]["Billno"].ToString();
                            }
                            else
                            {
                                txtLotNo.Text = "0";
                            }
                            ddlSalary.SelectedValue = ds1.Tables[0].Rows[0]["SalaryTypeID"].ToString();

                            //////DataSet getitemdistinct = objBs.editcuttingmasterrate(iCusID);
                            //////dCrt = (DataTable)ViewState["Data"];
                            //////for (int ii = 0; ii < getitemdistinct.Tables[0].Rows.Count; ii++)
                            //////{
                            ...
                            //////    string itemid = getitemdistinct.Tables[0].Rows[ii]["itemid"].ToString();
                            //////    string itemcode = getitemdistinct.Tables[0].Rows[ii]["itemcode"].ToString();
                            //////    string CutRate = getitemdistinct.Tables[0].Rows[ii]["CutRate"].ToString();

                            //////    // DataSet gettransjobworkerrate = objBs.editCustomerjobrate(iCusID, itemid, "Stc");
                            //////    // if (dCrt.Rows.Count == 0)
                            //////    {
                            //////        DataRow dr = dCrt.NewRow();
                            //////        dr["Itemid"] = itemid;
                            //////        dr["Itemname"] = itemcode;
                            //////        dr["cutrate"] = Convert.ToDouble(CutRate).ToString("0.00");
                            //////            // break;
        ...
        protected void process_click(object sender, EventArgs e)
        {
            ...
            dr["cutrate"] = Convert.ToDouble(txtcostperqty.Text).ToString("0.00");
        ...
        protected void Add_Click(object sender, EventArgs e)
        {
            ...
            DateTime dat = DateTime.ParseExact(DateTime.Now.ToString("dd/MM/yyyy"), "dd/MM/yyyy", CultureInfo.InvariantCulture);
            ...
            if (btnadd.Text == "Save")
            {
                if (ddlCDType.SelectedValue == "Credit Note")
                {
                    ...
                    int i = objBs.InsertCuttingMaster(txtcustomername.Text, txtmobileno.Text, txtphoneno.Text, txtarea.Text, txtaddress.Text, txtcity.Text,
                        txtpincode.Text, txtemail.Text, Grupid, ddlIsActive.SelectedValue, Convert.ToDouble(Credite),
                        Convert.ToDouble("0"), ddlCDType.SelectedValue, Convert.ToInt32("7"), Convert.ToInt32(txtCustomerid.Text),
                        Tinno, "tblAuditMaster_" + sTableName, lblUser.Text, "Vendor", ddlCDType.SelectedItem.Text, openingbalance,
                        txtCustomerid.Text, TextBox1.Text, txtCST.Text, txtPAN.Text, txtDesignation.Text, txtDelivery.Text, 0, 0, 0,
                        Convert.ToInt32(ddlPrice.SelectedValue), date, dat, Convert.ToInt32(txtCreditDays.Text), "0", txtprintname.Text, Convert.ToInt32(txtLotNo.Text), Convert.ToInt32(ddlSalary.SelectedValue), ddlSalary.SelectedItem.Text);
                    //Response.Redirect("../Accountsbootstrap/viewcuttingmaster.aspx");
                }
                else
                {
                    ...
                    int j = objBs.InsertCuttingMaster(txtcustomername.Text, txtmobileno.Text, txtphoneno.Text, txtarea.Text, txtaddress.Text,
                        txtcity.Text, txtpincode.Text, txtemail.Text, Grupid, ddlIsActive.SelectedValue, Convert.ToDouble("0"),
                        Convert.ToDouble(Debit), ddlCDType.SelectedValue, Convert.ToInt32("7"), Convert.ToInt32(txtCustomerid.Text),
                        Tinno, "tblAuditMaster_" + sTableName, lblUser.Text, "Vendor", ddlCDType.SelectedItem.Text, openingbalance,
                        txtCustomerid.Text, TextBox1.Text, txtCST.Text, txtPAN.Text, txtDesignation.Text, txtDelivery.Text, 0, 0, 0,
                        Convert.ToInt32(ddlPrice.SelectedValue), date, dat, Convert.ToInt32(txtCreditDays.Text), "0", txtprintname.Text, Convert.ToInt32(txtLotNo.Text), Convert.ToInt32(ddlSalary.SelectedValue), ddlSalary.SelectedItem.Text);
                ...
                for (int vLoop1 = 0; vLoop1 < gridprocesstype.Rows.Count; vLoop1++)
                {
                    ...
                    // Label = (Label)gridprocesstype.Rows[vLoop1].FindControl("");
                    TextBox lblcutrate = (TextBox)gridprocesstype.Rows[vLoop1].FindControl("lblcutrate");
                    if (lblcutrate.Text != "0" || lblcutrate.Text != "" || lblcutrate.Text != null)
                    {
                        iscc = objBs.insertTranscuttingmasterRate(lblitemid.Text, lblcutrate.Text);
                    }
            ...
            else
            {
                ...
                if (ddlCDType.SelectedValue == "Credit Note")
                {
                    ...
                    int iStatus = objBs.updateCuttingMaster(Convert.ToInt32(iCusID), txtcustomername.Text, txtmobileno.Text, txtphoneno.Text, txtarea.Text, txtaddress.Text, txtcity.Text, txtpincode.Text, txtemail.Text, Convert.ToInt32("7"), ddlIsActive.SelectedItem.Text, Convert.ToDouble(Credite), Convert.ToDouble("0"), ddlCDType.SelectedValue, Convert.ToInt32(txtCustomerid.Text), Tinno, "tblAuditMaster_" + sTableName, lblUser.Text, "Vendor", ddlCDType.SelectedItem.Text, openingbalance, txtCustomerid.Text, TextBox1.Text, txtCST.Text, txtPAN.Text, txtDesignation.Text, txtDelivery.Text, 0, 0, 0, Convert.ToInt32(ddlPrice.SelectedValue), date, dat, Convert.ToInt32(txtCreditDays.Text), "0", txtprintname.Text, Convert.ToInt32(ddlSalary.SelectedValue), ddlSalary.SelectedItem.Text);
                    // Response.Redirect("../Accountsbootstrap/viewcuttingmaster.aspx");
                }
                else
                {
                    string Debit = txtOBalance.Text;
                    int iStatus = objBs.updateCuttingMaster(Convert.ToInt32(iCusID), txtcustomername.Text, txtmobileno.Text, txtphoneno.Text, txtarea.Text, txtaddress.Text, txtcity.Text, txtpincode.Text, txtemail.Text, Convert.ToInt32("7"), ddlIsActive.SelectedItem.Text, Convert.ToDouble("0"), Convert.ToDouble(Debit), ddlCDType.SelectedValue, Convert.ToInt32(txtCustomerid.Text), Tinno, "tblAuditMaster_" + sTableName, lblUser.Text, "Vendor", ddlCDType.SelectedItem.Text, openingbalance, txtCustomerid.Text, TextBox1.Text, txtCST.Text, txtPAN.Text, txtDesignation.Text, txtDelivery.Text, 0, 0, 0, Convert.ToInt32(ddlPrice.SelectedValue), date, dat, Convert.ToInt32(txtCreditDays.Text), "0", txtprintname.Text, Convert.ToInt32(ddlSalary.SelectedValue), ddlSalary.SelectedItem.Text);

                }

                int idelete = objBs.deletecuttingrate(iCusID);
                int iscc = 0;
                for (int vLoop1 = 0; vLoop1 < gridprocesstype.Rows.Count; vLoop1++)
                {
                    Label lblitemid = (Label)gridprocesstype.Rows[vLoop1].FindControl("lblitemid");
                    ...
                    if (lblcutrate.Text != "0" || lblcutrate.Text != "" || lblcutrate.Text != null)
                    {
                        iscc = objBs.UpdateTranscuttingRate(lblitemid.Text, lblcutrate.Text, iCusID);
                    }
        ...
        protected void txtcustomername_TextChanged(object sender, EventArgs e)
        {
            ...
            if ((ds.Tables[0].Rows.Count) > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "myscript", "alert('This Cutter Name is all ready exist!');", true);
